...
# 今日、明日、明後日の降水確率をDadaFrameに代入 　ちょっと遊びで降水率をDetailに変更
df1 = pd.DataFrame(weather_json['forecasts'][0]['detail'],index=["今日"]) # index名を今日という文字列に設定
df2 = pd.DataFrame(weather_json['forecasts'][1]['detail'],index=["明日"]) # index名を明日という文字列に設定
df3 = pd.DataFrame(weather_json['forecasts'][2]['detail'],index=["明後日"]) # index名を明後日という文字列に設定
